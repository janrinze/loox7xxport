...
static int acx_s_proc_diag_output(char *buf, acx_device_t * adev)
{
	...
	if (adev->ops && adev->ops->s_proc_diag_output)
		p = adev->ops->s_proc_diag_output(p, adev);
		       ...
		       int *eof, void *data)
{
	...
	/* fill buf */
	length = 0;
	if (adev->ops && adev->ops->proc_eeprom_output) {
		acx_sem_lock(adev);
		length = adev->ops->proc_eeprom_output(buf, adev);
		acx_sem_unlock(adev);
	}
...
static int manage_proc_entries(struct ieee80211_hw *hw, int remove)
{
	...
	for (i = 0; i < ARRAY_SIZE(proc_files); i++) {
		snprintf(procbuf, sizeof(procbuf),
			 "driver/acx_%s%s", wiphy_name(adev->ieee->wiphy), proc_files[i]);
...
//		mode->num_channels = 0;// !!!! acx_chantable_size;
//		mode->num_rates = 0;// !!!! acx_g_ratetable_size;
//		mode->rates = NULL; // !!!! acx_g_ratetable;
...
//	err = ieee80211_register_hwmode(hw,mode);
	...
	ieee->wiphy->bands[IEEE80211_BAND_2GHZ] = &band_2GHz;
...
static int acx111_s_create_dma_regions(acx_device_t * adev)
{
	...
	if (adev->ops && adev->ops->s_create_hostdesc_queues) {
		if (OK != adev->ops->s_create_hostdesc_queues(adev))
			goto fail;
	}
	...
	if (adev->ops && adev->ops->create_desc_queues)
		adev->ops->create_desc_queues(adev, tx_queue_start, rx_queue_start);
...
//		memset(&status, 0, sizeof(status));
		...
		if (likely(skb)) {
			...
			status->flag = 0;
			status->rate_idx = rxbuf->phy_plcp_signal; // !!!! rate->rate_idx
			status->antenna = 1;
/*
#ifndef OLD_QUALITY
...
*/
			...
			{
				status->rate_idx = acx_plcp_get_bitrate_ofdm(rxbuf->phy_plcp_signal); // !!!! rate->rate_idx
			} else
			{
...
*/
firmware_image_t *acx_s_read_fw(acx_device_t *adev, const char *file,
				u32 * size)
{
	...
	if (!request_firmware(&fw_entry, file, adev->ops->get_dev(adev))) {
		printk("request: size: %d\n", fw_entry->size);
		*size = 8;
		if (fw_entry->size >= 8)
...
int acx_s_init_mac(acx_device_t * adev)
{
	...
	if (IS_PCI(adev)) {
		...
		// !!!!
		if (adev->ops && adev->ops->s_upload_radio)
		  adev->ops->s_upload_radio(adev);
	} else {
...
//		memcpy(adev->dev_addr, adev->ndev->dev_addr, ETH_ALEN);
	...
	if (adev->set_mask & GETSET_LED_POWER) {
		...
		acx_lock(adev, flags);
		if (adev->ops->l_power_led)
			adev->ops->l_power_led(adev, adev->led_power);
...
void acx_init_task_scheduler(acx_device_t * adev)
{
	...
	INIT_WORK(&adev->after_interrupt_task, adev->ops->interrupt_tasklet);
}
...
int acx_net_reset(struct ieee80211_hw *ieee)
{
	acx_device_t *adev = ieee2adev(ieee);
	FN_ENTER;
	if (adev->ops && adev->ops->s_reset_dev)
		adev->ops->s_reset_dev(adev);
	else
		TODO();
